...
export const useCourseOptions = (
  ...
  setDraftCourse: React.Dispatch<React.SetStateAction<{ [iso: string]: Course }>>,
  setModal: ModalSetter,
  weekKey: string
) => {
  // Get list of courses for a specific ISO date
  const getCoursesForDay = useCallback((iso: string): Course[] => {
    return tempCourses[weekKey]?.[iso] || [];
  }, [tempCourses, weekKey]);

  // Begin editing a course
  const startEdit = useCallback((iso: string, index: number, course: Course) => {
  ...
  // Confirm editing and save course
  const confirmEdit = useCallback((iso: string, index: number) => {
  ...
  // Cancel editing and optionally remove unsaved course
  const cancelEdit = useCallback((iso: string) => {
  ...
  // Delete a course for a given day
  const deleteCourse = useCallback((iso: string, index: number) => {
    const isEditingNow = Object.values(editing).some(i => i !== null);
    if (isEditingNow) {
      setModal({
        title: 'Advertencia',
    ...
    setTempCourses(prev => {
      const dayCourses = prev[weekKey]?.[iso] || [];
      const newCourses = [...dayCourses.slice(0, index), ...dayCourses.slice(index + 1)];
      return {
        ...prev,
        [weekKey]: {
          ...prev[weekKey],
