...
export default function CursosDashboard() {
  ...
  const mover = (setter: React.Dispatch<React.SetStateAction<number>>, lista: Curso[], direccion: number) => {
    setter(prev => {
  ...
  const renderizarCursos = (lista: Curso[], indice: number) => (
    <div className="flex gap-6 mt-4 transition-transform duration-300">
      {[0, 1].map(i => {
        ...
        return (
          <div key={curso.titulo} className="w-[500px] h-52 border-4 rounded-2xl flex items-center cursor-pointer" 
          style={{ borderColor: '#06D6A0', backgroundColor: "#141414" }} 
          onClick={() => router.push('/verCursos')}>
            <div className="flex-1 p-3">
              <div style={{ color: 'white', fontSize: 20, fontWeight: '500' }}>{curso.titulo}</div>
              <div style={{ color: '#999', fontSize: 12 }}>{curso.descripcion}</div>
              ...
              <div className="flex gap-2 mt-2 w-60">
                {Array.from({ length: curso.tareasTotales }, (_, idx) => (
                  <div key={idx} className={`flex-1 h-1 rounded-full ${idx < curso.tareasCompletadas ? "bg-[#06D6A0]" : "bg-gray-500"}`} />
                ))}
              </div>
            </div>
            <Image src={curso.imagen} alt={curso.titulo} width={50} height={50} className="object-cover rounded-xl ml-auto" />
          </div>
        );
      })}
    </div>
              ...
              <div className="flex items-center justify-between mt-4">
                ...
                <div className="flex items-center">
                  {Array.from({ length: Math.ceil(lista.length / 2) }, (_, i) => (
                    <div key={i} className={`w-3 h-3 rounded-full mx-1 ${i === indice ? 'bg-[#636364]' : 'bg-[#D9D9D9]'}`}></div>
                  ))}
                </div>
                <button className="p-2 bg-[#141414] rounded-full" onClick={() => mover(setIndice, lista, 1)}>
                  <ChevronRight />
