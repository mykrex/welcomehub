...
export default async function handler(
  ...
  try {
    ...
    if (nuevo_estado === "completado") {
      ...
      if (cursoError || !curso)
        throw cursoError ?? new Error("Curso no encontrado");

      if (curso.obligatorio) {
        const { data: retoObligatorio, error: errorRetoObligatorio } =
          await supabase
            .from("reto")
            .select("id_reto")
            .eq("descripcion_reto", "curso_obligatorio_completo")
            .single();
        if (errorRetoObligatorio || !retoObligatorio)
          throw errorRetoObligatorio ?? new Error("Reto no encontrado");
        ...
        if (cursoIds.length > 0) {
          ...
          const todosObligatoriosCompletados = cursosDetalles
            ...
            .every((cursoDet) => {
              const cursoUsuario = cursosUsuario.find(
                (cu) => cu.id_curso === cursoDet.id_curso
              );
          ...
          if (todosObligatoriosCompletados) {
            const { data: retoTodos, error: errorRetoTodos } = await supabase
              ...
              .single();
            if (errorRetoTodos || !retoTodos)
              throw errorRetoTodos ?? new Error("Reto no encontrado");
        ...
        const { data: retoOpcional, error: errorRetoOpcional } = await supabase
          ...
          .single();
        if (errorRetoOpcional || !retoOpcional)
          throw errorRetoOpcional ?? new Error("Reto no encontrado");
    ...
    return res
      .status(200)
      .json({ success: true, message: `Estado actualizado a ${nuevo_estado}` });
  } catch (error: unknown) {
    const message =
