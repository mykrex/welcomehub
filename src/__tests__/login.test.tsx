...
describe("HU-01: User Login", () => {
  test("1. Renders login form correctly", () => {
    ...
    expect(screen.getByPlaceholderText("usuario@neoris.mx")).toBeInTheDocument();
  ...
  test("4. Shows error if credentials are invalid on server", async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('/api/auth/login', {
      ...
      body: JSON.stringify({ email: 'usuario@neoris.mx', password: 'Contrasena1!' }),
    });
  });
  ...
  test("6. Doesn't allow form submission if fields are empty", () => {
    ...
    // Verify that fields are required
    expect(emailInput).toHaveAttribute('required');
    expect(passwordInput).toHaveAttribute('required');
    
    // Browser prevents submit if required fields are empty
    // We don't need to verify behavior as it's handled by the browser
  });
  ...
  test("7. 'Forgot password?' link points to /olvide_contrasena", () => {
    ...
    expect(link).toHaveClass("text-blue-500", "hover:text-blue-600", "underline");
  });
  ...
  test("8. Login button is present and has correct styles", () => {
    ...
    expect(button).toHaveClass("w-full", "bg-gradient-to-r", "from-blue-400", "to-blue-600");
  });
  ...
  test("10. Handles errors without specific message", async () => {
    (global.fetch as jest.Mock).mockResolvedValueOnce({
      ok: false,
      json: async () => ({}), // Without error field
    });
